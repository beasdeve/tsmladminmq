<?php
	...
	use App\Jobs\CreateProductCategory;
	use App\Jobs\CategoryUpdated;
	use App\Jobs\InActiveCategory;
	use App\Jobs\ActiveCategory;
	use JWTAuth;
	use Validator;
	use File; 
	use Storage;
	     ...
	     */
	   	...
	   	{ 
		    ...
		    {  
                ...
                // $destination='C:\xampp\htdocs/microservice/login/storage/app/public/images/product';
                $destination='/var/www/beas.in/public_html/tsml-microservice/login/storage/app/public/images/product'; 
                 

                $request->file('primary_image')->move($destination, $filename);
	        ...
	        $purpose = 'Category Created';

	        $res = (new AdminLogsService)->adminlogs($user,$sub,$purpose);

	        $input['cateId'] = $categoryData->id; 
	        $input['filename'] = $filename;

	        // dd($input);
	        CreateProductCategory::dispatch($input);
	    ...
	    */
	   	public function editCategory($catId)
	   	{
	   		...
	   		if (!empty($catData)) 
	   		{
              	...
              	$catdetails['category_dese'] = $catData->cat_dese;
	    ...
	    */
		   	...
		   	public function updateCategory(Request $request)
		   	{
				// dd($request->all());
				   ...
				   { 
	                ...
	                $purpose = 'Category Update';

	                $res = (new AdminLogsService)->adminlogs($user,$sub,$purpose);
	                 
	               	$updatecat['cateId'] = $srch_reg->id; 
	        		// $updatecat['filename'] = $filename;

	                CategoryUpdated::dispatch($updatecat);
	    ...
	    {  
	        ...
	        if(!empty($category))
	        {
	        	...
	        	if (!empty($checkForUse)) {
		            ...
		            $purpose = 'Category inactive';

		            $res = (new AdminLogsService)->adminlogs($user,$sub,$purpose);
		            $input['cateId'] = $category->id;
		            
		            InActiveCategory::dispatch($input);
	    ...
	    {  
	        ...
	        if(!empty($category))
	        {
	            ...
	            $user = Auth::user()->id; 
	            $sub = 'Category Active';
	            $purpose = 'Category Active';

	            $res = (new AdminLogsService)->adminlogs($user,$sub,$purpose);
	            $input['cateId'] = $category->id;
	            
	            ActiveCategory::dispatch($input);
