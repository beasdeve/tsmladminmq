...
use App\Jobs\ProductPriceStore;
use App\Jobs\ProductPriceUpdate;
use App\Jobs\ProductPriceDelete;
use App\Jobs\PriceExcelInsert;
use App\Jobs\PriceExcelUpdate;
use JWTAuth;
use Validator;
use File; 
use Storage;
    ...
    public function manageProPrice(Request $request)
    {
      ...
      try{
        ...
        if(empty($getthoprice))
        {
          ...
          $user = Auth::user()->id;
          $sub = 'Product Price Add';
          $purpose = 'Product price add';

          $res = (new AdminLogsService)->adminlogs($user,$sub,$purpose);

          $input['price_id'] = $freightsData->id; 
             

            // dd($input);
          ProductPriceStore::dispatch($input);
                    ...
                    foreach($sheetData as $k => $val)
                    {
                       ...
                       if(!empty($product_id) && !empty($category_id) & !empty($subcategory_id)){
                        ...
                        // dd($res);
                         ...
                         if(empty($res))   
                         {
                            $users['pro_id'] = $product_id->id;
                            $users['cat_id'] = $category_id->id;
                            $users['sub_cat_id'] = $subcategory_id->id;
                            $users['size'] = $val[3];
                            $users['BPT_Price'] = $val[4];
                            $users['Price_Premium_sing'] = $val[5];
                            $users['Price_Premium'] = $val[6];
                            $users['Misc_Expense'] = $val[7];
                            $users['CAM_Discount'] = $val[8];
                            $users['Interest_Rate'] = $val[9];
                            $users['type'] = $val[10]; 
                            // dd($users);
                            $freightsData = PriceCalculation::create($users);

                             

                              
  
                            PriceExcelInsert::dispatch($users);

                       }
                       else{
                                

                            $userup['BPT_Price'] = $val[4];
                            $userup['Price_Premium_sing'] = $val[5];
                            $userup['Price_Premium'] = $val[6];
                            $userup['Misc_Expense'] = $val[7];
                            $userup['CAM_Discount'] = $val[8];
                            $userup['Interest_Rate'] = $val[9];
                                

                            PriceCalculation::where('pro_id',$product_id->id)->where('cat_id',$category_id->id)->where('sub_cat_id',$subcategory_id->id)->where('size',$val[3])->where('type',$val[10])->update($userup);

                            // $user = Auth::user()->id;
                            // $sub = 'Product Price Update Excel';
                            // $purpose = 'Product price update excel';

                            // $res = (new AdminLogsService)->adminlogs($user,$sub,$purpose);
                            $userup['pro_id'] = $product_id->id;
                            $userup['cat_id'] = $category_id->id;
                            $userup['sub_cat_id'] = $subcategory_id->id;
                            $userup['size'] = $val[3];
                            $userup['type'] = $val[10];
                            // $user['price_id'] = $freightsData->id;  
 
                            PriceExcelUpdate::dispatch($userup);
    ...
    */
    public function updateThresholdProPrice(Request $request)
    {
      ...
      try{
        ...
        $input['price_id'] = $request->threshold_id; 
             

            // dd($input);
        ProductPriceUpdate::dispatch($input);
    ...
    public function deleteThresholdProPrice($ThresholdId)
    {
      try{ 
          ...
          if (!empty($ThresholdData))
          {
            ...
            $user = Auth::user()->id;
            $sub = 'Product Price Delete';
            $purpose = 'Product price delete';

            $res = (new AdminLogsService)->adminlogs($user,$sub,$purpose);

            $input['price_id'] = $ThresholdId; 
                 

                // dd($input);
            ProductPriceDelete::dispatch($input);
