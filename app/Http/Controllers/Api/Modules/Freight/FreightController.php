...
use App\Jobs\FreightCreate;
use App\Jobs\FreightUpdate;
use App\Jobs\FreightActive;
use App\Jobs\FreightInActive;
use App\Jobs\FreightDelete;
use App\Jobs\FreightImportExcelUpdate;
use App\Jobs\FreightImportExcelInsert;
use JWTAuth;
use Validator;
use DB; 
use File; 
...
class FreightController extends Controller
{
        ...
        if($excel)
        {
          ...
          foreach($sheetData as $k => $val)
          {
            ...
            { 
              ...
              $upfdata['pickup_from'] = $freightData->pickup_from;
              $upfdata['location'] = $freightData->location;
              $upfdata['destation_location'] = $freightData->destation_location;
              ...
              // $upfdata['freightid'] = $freightData->id; 

              // dd($input);
              FreightImportExcelUpdate::dispatch($upfdata);
            ...
            else
            {
              ...
              $createfdata['status'] = 1;
              $createfdata['manage_by'] = Auth::user()->id;

              
              $getFreight = Freights::create($createfdata);

 

              // dd($input);
              FreightImportExcelInsert::dispatch($createfdata);
   ...
   public function storeFreights(Request $request)
   {
          ...
          if (empty($getFreightD)) 
          {
            ...
            $input['freightid'] = $freightsData->id; 
             

            // dd($input);
            FreightCreate::dispatch($input);
    ...
    */
	public function updateFreights(Request $request)
	{
		\DB::beginTransaction();
          ...
          $update['freightid'] = $request->freights_id; 

          // dd($input);
          FreightUpdate::dispatch($update);
    ...
    */
	public function activeFreights($id)
	{
		...
		\DB::beginTransaction();
          ...
          $user = Auth::user()->id;
          $sub = 'Frieght Active';
          $purpose = 'Frieght active';

          $res = (new AdminLogsService)->adminlogs($user,$sub,$purpose);

          $input['freightid'] = $getFreight->id; 

          // dd($input);
          FreightActive::dispatch($input);
    ...
    */
	public function inactiveFreights($id)
	{
		...
		\DB::beginTransaction();
            ...
            $user = Auth::user()->id;
            $sub = 'Frieght Inactive';
            $purpose = 'Frieght inactive';
            ...
            // dd($input);
            FreightInActive::dispatch($input);
